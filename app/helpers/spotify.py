...
async def get_spotify_client(user_id: str, db: Session = Depends(get_db)) -> SpotifyClient:
    ...
    if not user or not user.spotify_token:
        raise HTTPException(
            status_code=401,
            detail="User not found or Spotify token not available"
        )
...
async def sync_user_spotify_data(user_id: str, db: Session) -> None:
    ...
    try:
        ...
        top_tracks_data = await spotify.current_user_top_tracks(limit=50, time_range="long_term")
        top_artists_data = await spotify.current_user_top_artists(limit=50, time_range="long_term")
        ...
        user_update = {
            ...
            "spotify_image_url": user_data["images"][0]["url"] if user_data.get("images") else None,
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Failed to sync Spotify data: {str(e)}"
        )
