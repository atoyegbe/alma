...
async def get_top_artists(
    ...
    return await music.get_user_top_artists(
        db=db, user_id=current_user.id, time_range=time_range, limit=limit
    )
...
async def get_top_genres(
    ...
    return await music.get_user_top_genres(
        db=db, user_id=current_user.id, time_range=time_range
    )
...
async def get_mutual_interests(
    ...
    if current_user.id == user_id:
        raise HTTPException(
            status_code=400, detail="Cannot get mutual interests with yourself"
        )

    return await music.get_mutual_music_interests(
        db=db, user1_id=current_user.id, user2_id=user_id
    )
