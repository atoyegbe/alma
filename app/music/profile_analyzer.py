...
class MusicProfileAnalyzer:
    ...
    async def analyze_listening_patterns(self, recent_tracks: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_played": dict(patterns["most_played"].most_common(10)),  # Top 10 tracks
    ...
    async def calculate_energy_score(self, audio_features: List[Dict[str, Any]]) -> int:
        ...
        return int(sum(track["energy"] * 100 for track in valid_features) / len(valid_features))
    
    async def calculate_danceability_score(self, audio_features: List[Dict[str, Any]]) -> int:
        ...
        return int(sum(track["danceability"] * 100 for track in valid_features) / len(valid_features))
    ...
    async def calculate_diversity_score(
        ...
        feature_score = sum(feature_scores) / len(feature_scores) if feature_scores else 50
    ...
    async def calculate_obscurity_score(
        ...
        """
        # Artist obscurity (40%)
        artist_scores = [100 - artist.get("popularity", 50) for artist in artists if artist]
        artist_obscurity = sum(artist_scores) / len(artist_scores) if artist_scores else 50
        ...
        # Mainstream ratio (20%)
        mainstream_threshold = 70  # Artists with popularity > 70 are considered mainstream
        mainstream_count = sum(1 for artist in artists if artist.get("popularity", 0) > mainstream_threshold)
    ...
    async def get_complete_profile_metrics(self) -> Metrics:
        ...
        top_tracks = await self.spotify.current_user_top_tracks(limit=50, time_range="long_term")
        top_artists = await self.spotify.current_user_top_artists(limit=50, time_range="long_term")
        ...
        audio_features = await self.spotify.audio_features(track_ids) if track_ids else []
        ...
        metrics = {
            "favorite_decades": await self.analyze_favorite_decades(top_tracks.get("items", [])),
            "listening_patterns": await self.analyze_listening_patterns(recent_tracks.get("items", [])),
            "energy_score": await self.calculate_energy_score(audio_features),
            "danceability_score": await self.calculate_danceability_score(audio_features),
